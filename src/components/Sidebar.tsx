...
import { CheckCircle2, Circle, Menu, X, ChevronDown } from "lucide-react";
...
export default function Sidebar() {
  ...
  // Support both new role-scoped route and legacy /playbooks route
  const rolePathMatch = pathname.match(/\/role\/([^/]+)\/playbooks\/([^/]+)\/(\d+)/);
  const legacyPathMatch = pathname.match(/\/playbooks\/([^/]+)\/(\d+)/);

  const roleSlug = rolePathMatch ? rolePathMatch[1] : null;
  const currentModuleId = rolePathMatch
    ? parseInt(rolePathMatch[3], 10)
    : legacyPathMatch
      ? parseInt(legacyPathMatch[2], 10)
      : null;

  const base = roleSlug
    ? `/role/${roleSlug}/playbooks/${playbook.slug}`
    : `/playbooks/${playbook.slug}`;

  const backHref = roleSlug ? `/role/${roleSlug}` : "/";
  const backLabel = roleSlug ? "← Back to hub" : "← All playbooks";
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
          {playbook.moduleSections.map((sectionName) => {
            ...
            const completedInSection = sectionModules.filter((m) => completedModules.includes(m.id)).length;
            const hasActiveModule = sectionModules.some((m) => pathname === `${base}/${m.id}`);
            ...
            return (
              <div key={sectionName} className="mb-1">
                <button
                  ...
                  className={cn(
                    "group w-full flex items-center justify-between px-3 py-2 rounded-md transition-colors",
                    hasActiveModule ? "text-accent" : "text-muted-foreground hover:text-foreground hover:bg-muted/50",
                  )}
                >
                  <span className="text-xs font-semibold uppercase tracking-wider">{sectionName}</span>
                  <div className="flex items-center gap-1.5 shrink-0">
                    {completedInSection > 0 && (
                      <span className="text-[10px] font-medium text-emerald-accent">
                          ...
                          return (
                            ...
                            >
                              ...
                              {isCompleted ? (
                                ...
                                <Circle className="w-[8px] h-[8px] mx-1 text-accent flex-shrink-0 fill-accent" />
                              ) : null}
                            </Link>
                          );
                        })}
